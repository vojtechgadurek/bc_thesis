...
Abstract:
Our work explored algorithms for finding the symmetric differences between two large sets that differ only in a few elements and their Bioinformatics application. We focused on performance and modularity from an implementation perspective and memory complexity from an algorithmic perspective. We achieved performant and modular code using advanced C\# features, mainly expression trees, allowing us to encode $120$ million $k$-mers per second. Finally, we improved the memory ratio memory size: elements in difference from $1$:$0.8$ to $1:1.25$ for $k$-mers.
